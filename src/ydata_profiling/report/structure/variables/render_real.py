...
def render_real(config: Settings, summary: dict) -> dict:
    ...
    if isinstance(summary.get("histogram", []), list):
        ...
        hist_caption = f"<strong>固定大小的直方图</strong> (bins={bins})"
    else:
        hist_data = histogram(config, *summary["histogram"])
        hist_caption = f"<strong>固定大小的直方图</strong> (bins={len(summary['histogram'][1]) - 1})"
    ...
    evs = Container(
        [
            FrequencyTable(
                ...
                name=f"最小 {config.n_extreme_obs} 个值",
                anchor_id=f"{varid}firstn",
                redact=False,
            ),
            FrequencyTable(
                template_variables["lastn_expanded"],
                name=f"最大 {config.n_extreme_obs} 个值",
